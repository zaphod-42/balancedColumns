(function($){
 ...
 function balancedColumns(wrapper, selector, width){
	...
	balancedColumns.prototype.createContainers=function(){
	    if($('.flex-container', this.wrapper).length==0) $(this.wrapper).append('<div class="flex-container"></div>');
      if($('.temp-container', this.wrapper).length==0) $(this.wrapper).append('<div class="temp-container"></div>');
	...
	balancedColumns.prototype.processChildren=function(){
	    ...
	    if(cell.find('img').length==0){
            ...
            cell.find('img:last').one("load error", function(e) {
                if(constr.temp_container.children().length==0) return;
	...
	balancedColumns.prototype.pickColumn=function(){
        ...
        heights.sort(function(a,b){return a[0]-b[0]});
        return heights[0][1];
  }
	balancedColumns.prototype.showDisplayNone=function(){
        var display_css_block={display:'block', visibility:'hidden', position:'absolute'};
        this.display_par=[];
        this.display_css=[];
        var fetch_css=function(par){
           return {display:par.css('display'), visibility:par.css('visibility'), position:par.css('position')};
        };
        var pars=this.wrapper.parents();
        constr=this;
        pars.each(function(){
           if(constr.wrapper.innerWidth()>0 || this.tagName.toLowerCase()=='body') return false;
	...
	balancedColumns.prototype.hideDisplayNone=function(){
        for(var i=0;i<this.display_par.length;i++) this.display_par[i].css(this.display_css[i]);
  }
})(jQuery);
